...
fn build_scores_table(results: String) -> HashMap<String, Team> {
    ...
    for r in results.lines() {
        ...
        if let Some(element) = scores.get_mut(&team_1_name) {
            element.goals_scored += team_1_score;
            element.goals_conceded += team_2_score;
        } else {
            scores.insert(team_1_name, Team { goals_scored: team_1_score, goals_conceded: team_2_score });
        }

        if let Some(element) = scores.get_mut(&team_2_name) {
            element.goals_scored += team_2_score;
            element.goals_conceded += team_1_score;
        } else {
